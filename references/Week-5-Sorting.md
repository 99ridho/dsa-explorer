...
## Week 5: Elementary Sorting Algorithms
...
**Sub-CPMK-1.5:** Students understand basic algorithms for searching and sorting problems. *(This session covers the sorting half; Week 7 covers searching.)*
...
The three elementary sorts share the same two-operation cost model (compares and exchanges) but optimize for different things: selection sort minimizes exchanges regardless of input order, insertion sort excels on nearly-sorted input by doing work proportional to how disordered the data actually is, and shellsort trades a more complex access pattern for a genuine improvement in asymptotic growth. None of the three is "best" in an absolute sense: the right choice depends on which operation is expensive and what the input actually looks like, the same cost-model thinking introduced in Week 1's complexity analysis.
