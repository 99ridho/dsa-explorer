...
- **`TopicPage`** is a two-column grid at `lg`: left is `<VisualizerShell key={slug}>`, right is shadcn `Tabs` (Real-World Usage by default, or Core Material) over `MarkdownContent`. At `lg` the `<article>` is capped at `100svh` minus the 3.5rem header and `main`'s `py-6`, with `overflow-hidden`, so the document never scrolls; the Code listing and the active `TabsContent` are the scrollers (SPEC §6, §12). The visualizer section has `overflow-y-auto` only as a fallback for viewports too short for canvas + Operation + Playback. Narrow viewports stack, visualizer first, and the document scrolls. The `key` is what resets visualizer state (and the selected tab) when navigating between topics. `e2e/topic-page-layout.spec.ts` is the executable form of this contract.
